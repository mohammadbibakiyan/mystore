            ...
            <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-grow">
                    {specifications.map((e,i)=><div className="flex items-center text-body-1" key={i}>
                        ...
                        <div className="border-0 border-b-2 border-solid w-full max-w-4xl border-gray-100">
                            <p className="color-gray-900">{e[1]}</p>
                        </div>
                    </div>)}
                    {!more&&<span className="text-body-2 text-blue-600 cursor-pointer" onClick={()=>setMore(true)}>بیشتر&gt;</span>}
                    {more&&<span className="text-body-2 text-blue-600 cursor-pointer" onClick={()=>setMore(false)}>بستن&gt;</span>}
                </div>
            </div>
        </div>
    )
